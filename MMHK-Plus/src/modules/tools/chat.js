MMHKPLUS.Chat = MMHKPLUS.PanelElement.extend({
	...
	_messageRenderer : function(message)
	{
        ...
        messageContent = messageContent.replace(new RegExp("(\\(\\s*([0-9]{1,3})\\s*:\\s*([0-9]{1,3}\\s*)\\))", "gi"), "<span style='color:blue;cursor:pointer;' onClick='MMHKPLUS.centerOn($2, $3, false);'>$1</span>");     
        messageContent = messageContent.replace(new RegExp("(\\(\\s*([0-9]{1,3})\\s*\,\\s*([0-9]{1,3}\\s*)\\))", "gi"), "<span style='color:blue;cursor:pointer;' onClick='MMHKPLUS.centerOn($2, $3, false);'>$1</span>");
        messageContent = messageContent.replace(/MMHKPLUS_ScoutPL\(([0-9a-zA-Z]+),([A-Za-z0-9_\-\s'"\?\!\w]+)\)/,"<span style='color:blue;cursor:pointer;' onClick='MMHKPLUS.getElement(\"Ajax\").getSpyReportContent(\"$1\", MMHKPLUS.getElement(\"AllianceSpys\", true)._openSpyReport);'>$2</span>");
